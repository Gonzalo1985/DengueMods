...
data.operativo <- data[which(data$Fecha >= "2024-07-28" & data$Fecha <= "2024-11-02"),]
...
final.ola.anterior <- which(cases.last.wave$Semana == "24/30")

inicio.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "24/31")
final.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "24/44")
...
#data.complete <- data.complete[53:66, ]
