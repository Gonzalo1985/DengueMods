...
Prcp.1m.Tucu <- c(0.0, 3.1, 3.1, 7.8, 7.8, 4.8, 4.7, 1.7, 11.0, 11.0, 101, 107, 99.0, 125, 109.1, 67.9, 69.2, 125.20, 87.90, 118.70, 114.60)
Prcp.1m.Resi <- c(9.0, 58.0, 58.0, 126, 118, 116, 71.0, 71.0, 43.0, 47.0, 74.2, 97.4, 78.4, 74.5, 154.5, 146.6, 243.6, 345.60, 266.40, 382.70, 277.70)
...
data.operativo <- data[which(data$Fecha >= "2024-07-28" & data$Fecha <= "2024-12-21"),]
...
cases.24.25 <- read_excel(paste0(path.ppal, "casos/Casos Temporada 24_25 a SE 51.xlsx"), sheet = 1)
...
final.ola.anterior <- which(cases.last.wave$Semana == "23/51")

inicio.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "24/31")
final.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "24/51")
...
tabla <- data.frame(Dates = seq.Date(as.Date("2024-07-28"), as.Date("2024-12-21"), 7),
...
#delete.last.rows <- which(cases.last.wave.complete$Semana == "24/52"):nrow(cases.last.wave.complete)
...
data.complete <- data.complete[50:75, ]
#data.complete$Casos <- data.complete$Casos * 10
