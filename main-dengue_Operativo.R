...
Prcp.1m.Tucu <- c(0.0, 3.1, 3.1, 7.8, 7.8, 4.8, 4.7, 1.7, 11.0, 11.0, 101, 107,
                  ...
                  134.4, 109.4, 104.0, 130.0, 51.0, 52.0, 183.0, 147.70, 177.70, 194.70, 60.70, 51.40)
Prcp.1m.Resi <- c(9.0, 58.0, 58.0, 126, 118, 116, 71.0, 71.0, 43.0, 47.0, 74.2,
                  ...
                  32.0, 0.3, 80.3, 80.3, 80.3, 86.0, 6.0, 6.0, 6.3, 0.30, 83.90, 89.60, 99.30, 99.30)
...
data.operativo <- data[which(data$Fecha >= "2024-07-28" & data$Fecha <= "2025-04-19"),]
...
cases.24.25 <- read_excel(paste0(path.ppal, "casos/Casos Temporada 24_25 a SE 16.xlsx"), sheet = 1)
...
#cases.last.wave$Casos <- cases.last.wave$Casos*2
...
final.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "25/16")
...
tabla <- data.frame(Dates = seq.Date(as.Date("2024-07-28"), as.Date("2025-04-19"), 7),
...
data.complete <- data.complete[50:92, ]
...
#data.for.print <- data.for.print[-seq(1,86,1),]
# ------------------------------------------------------------------------------
