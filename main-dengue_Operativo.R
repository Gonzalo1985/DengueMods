...
Prcp.1m.Tucu <- c(0.0, 3.1, 3.1, 7.8, 7.8, 4.8, 4.7, 1.7, 11.0, 11.0, 101, 107, 99.0, 125, 109.1, 67.9, 69.2)
Prcp.1m.Resi <- c(9.0, 58.0, 58.0, 126, 118, 116, 71.0, 71.0, 43.0, 47.0, 74.2, 97.4, 78.4, 74.5, 154.5, 146.6, 243.6)
...
data.operativo <- data[which(data$Fecha >= "2024-07-28" & data$Fecha <= "2024-11-23"),]
...
cases.24.25 <- read_excel(paste0(path.ppal, "casos/Casos Temporada 24_25 a SE 46.xlsx"), sheet = 1)
...
cases.last.wave <- cases.station[, c("ANIO_SEPI_MIN", "Autóctono", "Casos_probables")]
colnames(cases.last.wave) <- c("Semana", "Casos", "Probables")
...
final.ola.anterior <- which(cases.last.wave$Semana == "23/47")

inicio.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "24/31")
final.ola.actual <- which(cases.station$ANIO_SEPI_MIN == "24/47")
...
tabla <- data.frame(Dates = seq.Date(as.Date("2024-07-28"), as.Date("2024-11-23"), 7),
...
delete.last.rows <- which(cases.last.wave.complete$Semana == "24/47"):nrow(cases.last.wave.complete)
cases.last.wave.complete <- cases.last.wave.complete[-delete.last.rows,]
data.complete <- merge(data.complete, cases.last.wave.complete, by = "Semana", all = TRUE)

data.complete <- data.complete[50:71, ]
